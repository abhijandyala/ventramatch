...
| Database | **PostgreSQL** on **Railway** (RLS, `lib/db.ts` + `ventramatch.user_id`) | Managed Postgres, one connection string. Auth is separate (TBD: Auth.js, Clerk, etc.). |
...
│
├── proxy.ts                    <- Next 16 proxy (extend for auth when wired)
├── lib/
...
cd ventramatch
npm install
cp .env.example .env.local        # set DATABASE_URL (Railway or local Postgres)
# Apply schema once:
